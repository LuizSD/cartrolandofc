...
$(document).ready(function() {
	...
	function getStartPagination(teams_total) {

		var teams_per_page = 10,
				tota_pages = Math.ceil(teams_total / teams_per_page),
				start_pagination = [];

		for (var i=0; i < tota_pages; i++) {

			if (i > 0) {

				start_pagination.push(teams_per_page * i + 1);

			} else {

				start_pagination.push(teams_per_page * i);

			}

		}

		return start_pagination;
	...
	function showTeamsList(team, teams_total) {
		...
		var $team_input = $("#team"),
				...
				start_pagination = getStartPagination(teams_total);
